...
# Wrap order creation and execution in try/except blocks for resilience.
        ...
        for strategy in self.strategies:
            try:
                ...
                for signal in signals:
                    ...
                    if action != "HOLD":
                        order = Order(symbol, quantity if action == "BUY" else -quantity, price, status="PENDING")
    ...
    def execute_order(self, order: Order):
        try:
            ...
            print(f"Executed order: {order.symbol}, Quantity: {order.quantity}, Price: {order.price}, Status: {order.status}")
        except Exception as e:
